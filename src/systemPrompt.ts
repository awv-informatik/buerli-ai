...
3. When unsure of methods' exact parameters, fetch their docs (\`docs([...])\`) BEFORE calling them — batched, not one by one.
...
- Sketch work needs "SKETCHING"; multi-feature builds need the matching recipe — "recipes/parametric-part" (expressions + constraints + regeneration), "recipes/pattern-then-subtract" (N cutouts around an axis), "recipes/direct-modeling-eif" (programmatic one-shot construction), "recipes/verify-numerically" (how to check your work). Recipes encode the composed workflow WITH its pitfalls — imitating them is faster and safer than composing from method docs. Again: all in the ONE bulk \`docs([...])\` call.
